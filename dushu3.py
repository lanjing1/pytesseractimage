...
def img_sobel_binary(im, blur_sz):
    ...
    # OTSU提取二值图像
    ret, thd = cv2.threshold(sobel_8u, 200, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
...
def img_contour_extra(im):
    # 腐蚀和膨胀
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (20, 9))
...
def img_contour_select(ctrs, im):
    ...
    for item in ctrs:
        ...
        if len(approx) == 4 or len(approx) == 7:
            ...
            if (cv2.contourArea(approx)) > 3000 and (cv2.contourArea(approx) < 10000):
...
def img_tesseract_detect(c_rect, im):
    ...
    print(pytesseract.image_to_string(Image.open("wrap.jpg")))
    cv2.waitKey(0)
    return
